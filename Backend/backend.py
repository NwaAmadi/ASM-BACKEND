...
backend.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///asm.db'
...
@backend.route('/api/programs', methods=['GET'])
def get_programs():
...
def login():
    ...
    if bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
        ...
        return jsonify({"error": "Invalid username or password"}), 401
@backend.route('/api/programs/<int:id>', methods=['DELETE'])
...
@backend.route('/api/send-notification', methods=['POST'])
